...
function markPoint() {
  ...
  if (points.length >= 3) {
    alert("You have already marked 3 points. Please reset to mark again.");
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      if (points.length === 3) {
        document.getElementById("result").innerHTML += "<br>✅ All 3 points marked. You can now calculate the area.";
        document.getElementById("markBtn").disabled = true; // Disable mark button
      }
    },
    (err) => {
      alert("Error: " + err.message);
...
function calculateArea() {
  ...
  // You can choose to reset points here or leave them for user control
  // points = [];
...
function resetPoints() {
  points = [];
  document.getElementById("result").innerHTML = "🔄 Points reset. You can start marking again.";
  document.getElementById("markBtn").disabled = false;
}